    ...
    if ($leaderboardId === "") {
        ...
        while ($found_good_leaderboard == false) {
            if ($last == 0)
                ...
                $lastLeaderboardElemenent = $leaderboardReader->xpath("/response/leaderboard[last()-" . $last .
                    "]/lbid");
   ...
   OR throne_players.last_updated IS NULL);');
    ...
    try {
        ...
        foreach ($result as $row) {
            ...
            try {
                global $steam_apikey;
                $jsonUserData = get_data("http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=" .
                    $steam_apikey . "&steamids=" . $row['steamId']);
                $user = json_decode($jsonUserData, true);
                ...
                // Log the update.
                echo '[' . $c . '/' . $t . '] Updated ' . $row['steamId'] . " as " .
                    $user["response"]["players"][0]["personaname"] . "\n";
            } catch (Exception $e) {
                echo '[' . $c . '/' . $t . '] Failed to update ' . $row['steamId'] . ' due to ' . $e->getMessage() .
                    "\n";
                echo '[' . $c . '/' . $t . "]   Pulled from: " .
                    "http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=" . $steam_apikey .
...
function update_twitch() {
    ...
    try {
        ...
        foreach ($streams['streams'] as $stream) {
            ...
            $stmt->execute(array(':name' => $stream['channel']['name'], ':status' => $stream['channel']['status'], ':viewers' => $stream['viewers'], ':preview' => str_replace("http://",
                "https://", $stream['preview']['small'])));
        }
